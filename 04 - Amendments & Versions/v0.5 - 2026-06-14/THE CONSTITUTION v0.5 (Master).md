# THE CONSTITUTION — Master (v0.5)
### The one rule book every AI tool I run or ship obeys. The single source of truth — LIVE.
*v0.5 adds Operational Postures (Draft/Prod) + Part 13 The Judiciary & Advisory Council (`/council`). v0.4 completed the moral core (✦ The Soul).*
...
## 6 · DEPLOYMENT & SECURE LAUNCH  *(NEW — web · iOS · Android · macOS/desktop)*
...
## 13 · THE JUDICIARY & ADVISORY COUNCIL
1. **The Council** is the Constitution's deliberative branch — an in-session panel of diverse-perspective sub-agents (First-Principles · Risk/Contrarian · Executor · Constitution-Counsel → Chairman) that reads this Constitution and returns one compliant recommended path. Invoke: **`/council "<question>"`**. Cheap, no servers; the full 6-model LLM Council web app is an optional manual escalation for the biggest calls.
2. **Mandatory deliberation (in PROD posture):** before modifying database schemas/migrations/RLS, altering authentication/authorization, or refactoring **more than 3 core files**, convene `/council` and weigh its recommendation before proceeding. In DRAFT posture this is optional (speed).
3. **Adjudication:** if a rule conflict blocks a needed feature, convene `/council "interpret <rule> in the context of <X>"` to find a compliant path before escalating to me.
4. **Advisory, not a gate:** the Council recommends; **I keep the final word.** It never hard-blocks and never overrides a Hard Line, the Soul, or a Guardian.
...
*v0.5 — LIVE (2026-06-14). Adds Draft/Prod Postures (`~/.claude/posture.sh`) + Part 13 The Judiciary & Advisory Council (`/council`). Enforced by hooks + 5 posture-aware Guardians + 5 Police agents, wired into `~/.claude/`. Prior versions frozen in `04 - Amendments & Versions`. Amend via the snapshot workflow.*
